...
bool runGPU(unsigned int& done_count, std::string& tmp_ln, std::ifstream& inStream, std::vector<std::string>& LINES, thread_struct* args, FILE*& inFile, FILE*& outFile) 
{
	...
	split(tokens, LINES.at(0), " ");
	int row, col;
	row = LINES.size();
	col = tokens.size();
	if (row < 1 || col < 1) return false; // Empty Exception
...
void doMetaAnalysis()
{
	...
	try
	{
		...
		// No GPU  or  Running with GPU Fails
		if (GPU_EXISTS == false ||
			runGPU(done_count, tmp_ln, inStream, LINES, args, inFile, outFile) == false) 
		{
			runCPU(done_count, tmp_ln,
				inStream, LINES,
				args, inFile, outFile);
		}
	}
